...
[string]$AzModulePath = Join-Path $PSScriptRoot '/Modules' # Replace with the path to the Az modules
[string]$templateFile = Join-Path $PSScriptRoot '/Templates/ARM/myAksInfra.json' # Replace with the path to the ARM or BICEP template file
[string]$vnetTemplateFile = Join-Path $PSScriptRoot 'Templates/ARM/vnet.json' # Replace with the path to the ARM or BICEP template file
[string]$natTemplateFile = Join-Path $PSScriptRoot 'Templates/ARM/nat.json' # Replace with the path to the ARM or BICEP template file
[string]$miTemplateFileName = Join-Path $PSScriptRoot 'Templates/ARM/mi.json' # Replace with the path to the ARM or BICEP template file
...
[string]$vnetResourceGroupName = $rgName
...
[string]$natGatewayResourceGroup = $rgName
...
Write-Output "-------------------------------------------------------------------------"
Write-Output "|Step 3: Configure the vNet, subnet, and NAT gateway for the AKS cluster|"
Write-Output "-------------------------------------------------------------------------`n"
...
if ($choiceNat -eq "n") {
    ...
    $choiceNewNat = Read-Host -Prompt "Do you want to create a new NAT gateway (n) or use the one in $rgName (e)? (Type 'n' or 'e')"

    if ($choiceNewNat -eq "e") {

        $natGatewayName = Get-AzNatGateway -ResourceGroupName $rgName | Select-Object Name -ExpandProperty Name
        $publicIpName = Get-AzPublicIpAddress -ResourceGroupName $rgName | Select-Object Name -ExpandProperty Name
...
$choiceVnetRg = Read-Host -Prompt "Use the same resource group for the vNet as the AKS cluster? (y/n) "
...
if ($choiceVnet -eq "e") {
    ...
    try {
        ...
        Write-Progress -Activity "Configuring the virtual network, subnet, and NAT gateway..." -Status "$counter/5" -PercentComplete ($counter / 5 * 100)
        
        aksNetworkConfig -vnetResourceGroupName $rgName `
            -vnetName $vnetName `
            -vnetRange $vnetRange `
            -snetName $snetName `
            -snetRange $snetRange `
            -natGatewayName $natGatewayName `
            -natGatewayResourceGroup $rgName `
            -publicIpName $publicIpName `
    ...
    try {
        ...
        Write-Progress -Activity "Creating the virtual network, subnet, and NAT gateway..." -Status "$counter/5" -PercentComplete ($counter / 5 * 100)
